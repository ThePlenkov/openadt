...
| --- | --- | --- |
| `verify-fixtures-only` flagged a full destination id (`ABC_200_USER_EN`) in a narrative memory file | The fixture form passes the fixture SID guard, but the narrative-memory guard bans ANY destination id (including fixtures) — see `scripts/verify-agent-memory.ts:44` | When in doubt, describe the shape (`<SID>_<client>_<user>_<lang>`), never paste a concrete id — even a fixture. |
