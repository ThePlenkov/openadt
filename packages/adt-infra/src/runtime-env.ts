...
export function loadOpenAdtRuntimePaths(opts?: { configPath?: string }): OpenAdtRuntimePaths {
...
function tomlField(nested: string | undefined, legacy: string | undefined): string | undefined {
...
export function resolveWindowsEnvDefaults(view: Env): WindowsEnvDefaults {
  const home = homedir()
  const localAppData =
    view.string({ name: envVar('LOCALAPPDATA') }) ?? join(home, 'AppData', 'Local')
  const appData = view.string({ name: envVar('APPDATA') }) ?? join(home, 'AppData', 'Roaming')
  const temp = view.string({ name: envVar('TEMP') }) ?? join(localAppData, 'Temp')
  const systemRoot =
...
export function ensureMinimalProcessEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
...
export function isVsCodeAdtWorkspacePath(workspace: WorkspacePath): boolean {
  ...
  return (
    normalized.includes('workspacestorage') && normalized.includes('sapse.adt-vscode/adtworkspace')
  )
}
