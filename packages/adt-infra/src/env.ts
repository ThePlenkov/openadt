...
export class Env {
  ...
  private findKey(upperName: string): string {
    return Object.keys(this.env).find((k) => k.toUpperCase() === upperName) ?? upperName
  }
}
...
function pathContains(current: string, dir: string, sep: string): boolean {
  return current.split(sep).some((entry) => entry.toLowerCase() === dir.toLowerCase())
}

function validateInteger(name: EnvVarName, raw: string, min?: number, max?: number): number {
...
function assertRange(name: EnvVarName, value: number, min?: number, max?: number): void {
