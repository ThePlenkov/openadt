...
function listJavaPackages(): Set<string> {
  ...
  for (const module of readdirSync(appsDir)) {
    const javaRoot = join(appsDir, module, 'src', 'main', 'java', 'org', 'openadt')
...
function hasPackageInfo(pkgPath: string): boolean {
  ...
  for (const module of readdirSync(appsDir)) {
    const info = join(appsDir, module, 'src', 'main', 'java', slashPkg, 'package-info.java')
...
if (errors.length > 0) {
  console.error('verify-package-docs failed:\n' + errors.map((e) => `  - ${e}`).join('\n'))
  process.exit(1)
}
