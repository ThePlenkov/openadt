...
describe('planIdFromRelPath', () => {
  test('strips .plan.md suffix', () => {
    expect(planIdFromRelPath('.cursor/plans/mcp_shared_backend_en_5e8e195f.plan.md')).toBe(
      'mcp_shared_backend_en_5e8e195f'
    )
  })

  test('rejects non-plan paths', () => {
    expect(() => planIdFromRelPath('.cursor/plans/README.md')).toThrow('Not a Cursor plan file')
  })
})
...
describe('parsePlanFrontmatter', () => {
  ...
  test('throws on missing name', () => {
    expect(() => parsePlanFrontmatter({ frontmatter: 'overview: x\n' })).toThrow(/missing name/)
  })
...
describe('parsePlanContent', () => {
  test('parses tracked Cursor plan fixture', () => {
    const fixture = join(import.meta.dir, '../.cursor/plans/mcp_shared_backend_en_5e8e195f.plan.md')
  ...
  test('throws a clear error when the file does not exist', () => {
    ...
    try {
      expect(() => parsePlanFile({ absPath: join(dir, 'missing.plan.md'), root: dir })).toThrow(
        /Plan file not found/
      )
    } finally {
...
describe('buildIssueBody', () => {
  test('includes overview, tasks, markers, and blob link', () => {
    ...
    expect(body).toContain('<!-- openadt-plan-id: mcp_shared_backend_en_5e8e195f -->')
...
describe('listAllPlanFiles (recursive)', () => {
  test('finds plan files in nested subdirectories', () => {
    ...
    try {
      ...
      writeFileSync(join(plansDir, 'top.plan.md'), '---\nname: top\noverview: o\n---\nbody')
      writeFileSync(
...
describe('resolveMode', () => {
  test('rejects unknown invocation with usage()', () => {
    ...
    try {
      ...
      try {
        expect(() =>
          resolveMode({ dryRun: false, fromPush: false, syncAll: false, fileFlag: -1 }, [], root)
        ).toThrow('__exit__')
