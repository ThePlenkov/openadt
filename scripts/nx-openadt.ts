...
function findDevJar(): string {
  if (!existsSync(targetDir)) {
    console.error('Missing apps/openadt-cli/target. Run: nx package openadt-cli')
    process.exit(1)
  }
  ...
  if (jars.length === 0) {
    console.error('No openadt-*.jar in apps/openadt-cli/target. Run: nx package openadt-cli')
    process.exit(1)
  }
  return jars[0]!
}
...
const VALUE_FLAGS = new Set(['profile', 'config', 'collection', 'category', 'format'])
...
function useFatJar(profile: string | undefined, args: string[]): boolean {
  ...
  if (subcommand === 'auth' || subcommand === 'discovery' || subcommand === 'transports') {
    return false
  }
...
const profile = parseProfile(args) ?? normalizeProfile(process.env.OPENADT_PROFILE)
