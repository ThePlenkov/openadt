...
function extractFrontmatterAndBody(opts: PlanContentOptions): {
  ...
  const match = opts.content.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/)
  if (!match) {
    throw new Error('Plan file must start with YAML frontmatter delimited by ---')
  }
...
async function findOpenIssueForPlan(opts: FindIssueOptions): Promise<IssueRef | undefined> {
...
function isMissingPlanError(error: unknown): boolean {
  return error instanceof Error && error.message.startsWith('Plan file not found:')
}
