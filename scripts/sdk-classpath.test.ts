...
import { RUNTIME_SAP_LIB_MIN_JARS, countSapJars, resolveSapBundleDirs } from './sdk-classpath.ts'
...
describe('countSapJars', () => {
  test('returns zero for missing directory', () => {
    expect(countSapJars(join(tmpdir(), 'openadt-missing-' + Date.now()))).toBe(0)
  })
