...
const prereleaseIdArg = process.argv.find((a) => a.startsWith('--prerelease-id='))
const prereleaseId = prereleaseIdArg ? prereleaseIdArg.slice('--prerelease-id='.length).trim() : ''

const PRE_BUMPS = new Set(['prerelease', 'prepatch', 'preminor', 'premajor'])
const ALLOWED_PRE_IDS = new Set(['rc', 'beta', 'alpha'])

if (!bumpArg) {
  console.error('Usage: bun tools/release-version/src/main.ts --bump=patch|minor|major|...')
  process.exit(1)
}
...
function updateScoop(version: string): void {
  bumpManifest({
    ...
    version,
    url: (v) => `https://github.com/abapify/openadt/releases/download/v${v}/openadt-${v}.zip`,
...
function bumpManifest(target: BumpTarget): void {
  ...
  text = text.replace(/sha256 "[^"]+"/, 'sha256 "PLACEHOLDER_RUN_PACKAGE_RELEASE"')
...
if (requiresPrereleaseId(bumpArg, baseVersion) && !prereleaseId) {
  console.error(`--prerelease-id is required for bump=${bumpArg} (use rc, beta, or alpha)`)
  process.exit(1)
}
if (prereleaseId && !ALLOWED_PRE_IDS.has(prereleaseId)) {
  console.error(`Invalid --prerelease-id=${prereleaseId} (use rc, beta, or alpha)`)
  process.exit(1)
}

const nextVersion = formatVersion(bumpVersion(baseVersion, bumpArg, prereleaseId))
...
updateHomebrewMcp(nextVersion)
updateScoopMcp(nextVersion)
