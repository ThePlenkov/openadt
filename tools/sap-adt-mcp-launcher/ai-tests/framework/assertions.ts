...
function mcpRepliedCheck(payload: ToolCallPayload): AssertCheck {
  ...
  return {
    ...
    actual: hasBody
      ? `response body (${payload.contentText.length} chars)`
      : "empty response body",
    passed: hasBody,
  };
}
...
function contentContainsChecks(
  rule: string | string[],
  payload: ToolCallPayload,
): AssertCheck[] {
  ...
  return needles.map((needle) => ({
    ...
    actual: payload.contentText.includes(needle)
      ? `found "${needle}"`
      : `missing "${needle}"`,
...
function destinationsIncludeCheck(
  destination: string,
  payload: ToolCallPayload,
): AssertCheck {
  ...
  return {
    ...
    actual: found
      ? `found "${destination}" in response`
      : `"${destination}" not in response text`,
    passed: found,
  };
}
