 ...
 */
import { locateAdtLs } from '../client/locate.ts'
import {
  parseBridgeArgv,
  parseListArgv,
...
import {
  ...
  writeEndpoint,
} from '../service/backend/endpoint-store.ts'
import {
  ensureSharedBackend,
  type EnsureSharedBackendResult,
} from '../service/backend/ensure-backend.ts'
import { resolveDestinationImport } from '../service/import/gui-import.ts'
import { createMcpLog, eclipseWorkspaceLogPath } from '../infra/log.ts'
import { isVsCodeAdtWorkspacePath, type WorkspacePath } from '../infra/runtime-env.ts'
import { connectAdtLanguageServer, disposeLspSession } from '../client/lsp-client.ts'
import { killProcessTree } from '../infra/process.ts'
import {
  mcpHttpClientConfig,
  generateMcpToken,
  isPortInUseMessage,
...
import { DEFAULT_IMPORT_FROM, MARKETPLACE_URL, type McpServeConfig } from '../config/types.ts'
import { createStdioMcpBridge, McpHttpEndpoint, type StdioMcpBridge } from '../mcp/stdio-proxy.ts'
import {
  connectionRequester,
  HttpReadBackend,
  LspReadBackend,
  prewarm,
  readEnabled,
} from '../service/read/index.ts'
import { startReadAuxServer, type ReadAuxServer } from '../service/read/read-server.ts'
...
function printImportNotices(
  ...
  console.error(`Imported ${gui.imported.length} destination(s) from ${via}: ${ids}`)
  if (gui.fileUris.length > 0) {
    console.error(`Registered ${gui.fileUris.length} destination file(s) for adt-lsc.`)
...
function printServeState(state: ServerState, cfg: McpServeConfig): void {
  ...
  console.log(`Extension: ${state.extensionVersion} · Workspace: ${cfg.workspace}`)
...
async function shortCircuitIfExistingDaemon(cfg: McpServeConfig): Promise<boolean> {
...
function startStdioBridgeIfNeeded(cfg: McpServeConfig): StdioMcpBridge | undefined {
...
async function prepareStandaloneBackend(
  ...
  const gui = resolveDestinationImport(cfg.workspace, cfg.importFrom, cfg.explicitWorkspace)
...
function buildStandaloneReadBackend(session: LspSession): LspReadBackend | undefined {
...
function prewarmIfNeeded(
  ...
  void Promise.allSettled(gui.imported.map((d) => prewarm(req, { destination: d.id })))
}
...
function exitCodeForEnsureError(err: unknown, bridge: StdioMcpBridge): Promise<number> {
  const code = (err as NodeJS.ErrnoException).code
  const message = `[openadt-mcp] ${formatError(err)}`
  if (code === 'OPENADT_MCP_AMBIGUOUS') return failStdioAndExit(bridge, EXIT_AMBIGUOUS, message)
  if (code === 'OPENADT_MCP_TIMEOUT') return failStdioAndExit(bridge, EXIT_LOCK_TIMEOUT, message)
  if (code === 'OPENADT_MCP_SPAWN_FAILED')
    return failStdioAndExit(bridge, EXIT_SPAWN_FAILED, message)
  return failStdioAndExit(bridge, EXIT_LSP_MCP, `ensure backend failed: ${formatError(err)}`)
}
...
function logImportDiagnostics(
  ...
  console.error(`adt-lsc Eclipse log: ${eclipseWorkspaceLogPath(gui.workspace)}`)
  log.info(`import source: ${gui.importSource ?? cfg.importFrom}`)
  log.info(`destinations store: ${gui.destinationsStorePath ?? '(none)'}`)
  log.info(`workspace folders: ${gui.workspaceFolderUris.join(', ') || '(none)'}`)
}
...
async function startMcpHttp(
  ...
  try {
    ...
    if (!ready) {
      throw new Error(`MCP HTTP not ready at ${mcpUrl(started.port)} within timeout`)
    }
  } catch (err) {
    options.log?.error(`MCP HTTP not ready at port ${started.port}: ${formatError(err)}`)
    throw err
  }
  return started
}
...
async function runStdioBridgeOrHttpLoop(
  ...
  const runPromise = bridge.run(McpHttpEndpoint.forConfig(started.port, started.token))
  await Promise.race([runPromise, waitForIdleHttpServe(session).then(() => runPromise)])
}
...
function failServeError(
  ...
  if (isPortInUseMessage(msg)) {
    return failStdioAndExit(bridge, EXIT_PORT_IN_USE, `Port ${cfg.port} is already in use.`)
  }
  return failStdioAndExit(bridge, EXIT_LSP_MCP, `adtLs/mcp/startMCPServer failed: ${msg}`)
}

type ConnectResult = { session: LspSession } | { exit: number; message: string }
...
async function connectLanguageServer(
  ...
  try {
    ...
    if (destinationIds.length > 0) {
      console.error('[openadt-mcp] SAP logon in progress — approve SSO / Secure Login if prompted.')
    }
...
async function cmdStatus(argv: string[]): Promise<number> {
  const { port: requestedPort, token: explicitToken, json } = parseStatusArgv(argv)
...
async function cmdBridge(argv: string[]): Promise<number> {
  ...
  if (!json) {
    console.error(`[openadt-mcp] Attached to existing backend at ${record.url}`)
  }
...
const KNOWN_COMMANDS = ['serve', 'stop', 'bridge', 'status', 'list', 'print-config'] as const
...
function unknownCommand(name: string, argv: string[]): void {
  ...
  if (argv.includes('--stdio') && suggestion) {
    console.error(`Hint: your MCP client command should be \`openadt mcp ${suggestion} --stdio\`.`)
  }
  console.error('')
  usage()
}
