...
describe('isVsCodeAdtWorkspacePath', () => {
  ...
  test('rejects openadt default workspace', () => {
    expect(
      isVsCodeAdtWorkspacePath('C:\\Users\\me\\.openadt\\adt-ls-workspace' as WorkspacePath)
    ).toBe(false)
  })
})
...
describe('Env', () => {
  ...
  test('string returns trimmed value', () => {
    ...
    try {
      expect(processEnv().string({ name: envVar('OPENADT_TEST_GETENV') })).toBe('hello')
    } finally {
  ...
  test('set updates keyByUpper so subsequent lookups hit the new value', () => {
    ...
    expect(env.getTrimmed(envVar('LOCALAPPDATA'))).toBe('C:\\Users\\me\\AppData\\Local')
    expect(env.getTrimmed(envVar('localappdata'))).toBe('C:\\Users\\me\\AppData\\Local')
...
describe('loadOpenAdtRuntimePaths', () => {
  ...
  test('prefers nested runtime value over legacy top-level', () => {
    withTomlFile(
      ...
      (path) => {
        expect(loadOpenAdtRuntimePaths({ configPath: path }).jcoNativeDir).toBe('C:\\new\\jco')
      }
    )
  })
})
