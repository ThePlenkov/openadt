...
import { buildAdtLscSpawnRuntime } from '@openadt/adt-infra'
...
async function drainChildStreams(child: ChildProcessWithoutNullStreams): Promise<void> {
...
import { isTruthyEnv } from '@openadt/adt-infra'
const explicitPort = parseExplicitPort(process.env.OPENADT_MCP_PORT?.trim())
const restartArgs = isTruthyEnv(process.env.OPENADT_MCP_RESTART) ? ['--restart'] : []
const serveArgs = [
  'serve',
  '--stdio',
...
const child: ChildProcessWithoutNullStreams = spawn(runtime, [launcher, ...serveArgs], {
  cwd: process.cwd(),
