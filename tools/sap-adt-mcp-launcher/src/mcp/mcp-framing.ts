...
export class McpFrameDecoder extends McpDecoder {
  ...
  private processFrame(flush: boolean): void {
    ...
    if (this.buffer.length > 0) {
      throw new Error(`Incomplete MCP frame (${this.buffer.length} trailing bytes after stdin end)`)
    }
  }
...
export class McpNdjsonDecoder extends McpDecoder {
  ...
  private handleTrailing(): void {
    ...
    throw new Error(`Non-JSON trailing data in NDJSON stream: ${trimmed.slice(0, 100)}`)
  }
}
...
class InvalidNdjsonError extends Error {
  ...
  constructor(cause: unknown) {
    super(
      `Invalid JSON in NDJSON stream: ${cause instanceof Error ? cause.message : String(cause)}`
    )
  }
}
...
export class McpStdioEncoder extends Transform {
  ...
  private encodeBody(body: string): Buffer {
    return this.mode === 'ndjson' ? encodeNdjsonLine(body) : frameMcpMessage(body)
  }

  override _transform(body: string, _encoding: BufferEncoding, callback: TransformCallback): void {
    try {
