...
export function adtScenariosDir(root: string): string {
  return join(root, 'scenarios-adt')
}
...
const CODE_RE = /^(mcp|adt)-\d+$/
const FILE_RE = /^(mcp|adt)-\d+-[\w-]+\.md$/
...
export function normalizeScenarioCode(raw: string): ScenarioCode {
  ...
  if (!CODE_RE.test(code)) {
    throw new Error(`Invalid scenario code "${raw}" (expected mcp-N or adt-N)`)
  }
...
export function loadScenarios(root: string): Scenario[] {
  // Load from scenarios/ directory (mcp-* scenarios)
  const mcpDir = scenariosDir(root)
  const mcpScenarios = existsSync(mcpDir) ? loadScenariosFromDir(mcpDir) : []

  // Load from scenarios-adt/ directory (adt-* scenarios)
  const adtDir = adtScenariosDir(root)
  const adtScenarios = existsSync(adtDir) ? loadScenariosFromDir(adtDir) : []

  const out = [...mcpScenarios, ...adtScenarios]
...
function codeOrder(code: ScenarioCode): number {
  const prefix = code.startsWith('mcp-') ? 'mcp-' : 'adt-'
  return Number(code.slice(prefix.length))
}

/** Match by stable code (`mcp-3`, `adt-1`) or slug id (`search-objects`). */
export function filterScenarios(all: Scenario[], selector: string | undefined): Scenario[] {
  ...
  if (hit.length === 0) {
    throw new Error(
      `Unknown scenario: ${selector} (use mcp-N, adt-N, or slug id; bun run mcp:ai-tests -- --list)`
    )
  }
  return hit
}
