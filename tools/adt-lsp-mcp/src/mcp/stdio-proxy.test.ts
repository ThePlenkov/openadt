...
function mockStdoutCapture(): {
  ...
  process.stdout.write = ((chunk: string | Uint8Array, cb?: (err?: Error | null) => void) => {
...
describe('parseMcpHttpResponseBody', () => {
  test('parses application/json body', () => {
    ...
    expect(parseMcpHttpResponseBody({ contentType: 'application/json', body })).toEqual([body])
  })

  test('parses SSE data lines', () => {
    const sse = 'event: message\r\ndata: {"jsonrpc":"2.0","id":1,"result":{}}\r\n\r\n'
    expect(parseMcpHttpResponseBody({ contentType: 'text/event-stream', body: sse })).toEqual([
...
describe('jsonRpcErrorResponse', () => {
  test('includes request id', () => {
    const req = JsonRpcRequest.parse('{"jsonrpc":"2.0","id":42,"method":"tools/list"}')!
  ...
  test('skips notifications without id', () => {
    expect(
      JsonRpcRequest.parse('{"jsonrpc":"2.0","method":"notifications/initialized"}')
    ).toBeUndefined()
  })
})
...
describe('createStdioMcpBridge', () => {
  ...
  test('run forwards queued initialize to HTTP', async () => {
    ...
    try {
      ...
      const runPromise = bridge.run(McpHttpEndpoint.forConfig(2236, 'test-token'))
      process.stdin.emit('end')
      await runPromise
...
function mockSessionFetch(): {
  ...
  const toolsPayload = '{"jsonrpc":"2.0","id":2,"result":{"tools":[{"name":"t1"}]}}'
