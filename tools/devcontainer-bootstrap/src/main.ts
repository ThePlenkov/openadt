...
function writeDevcontainerRuntime(path: string, runtime: DevcontainerRuntimePaths): void {
  mkdirSync(dirname(path), { recursive: true })
  const lines = [
    'version = 1',
    '',
...
async function main(): Promise<void> {
  ...
  if (!existsSync(sourceRoot)) {
    if (args.skipIfMissing) {
      console.log(`SAP archives not found under ${sourceRoot} — skipping native archive bootstrap.`)
  ...
  const effectiveSourceRoot = existsSync(sourceRoot)
    ? sourceRoot
    : resolve(await promptValue('Enter source folder with SAP archives', sourceRoot))
